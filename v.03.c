    ...
    */
{
    ...
    {
        player1Start = 1;
        printf("%s is red and computer is yellow.", player1);
        printf("\n");
    }
    else
    {
        ...
        botStart = 1;
        printf("bot is red and %s is yellow.", player1);
        printf("\n");
    }
    display(gameBoard);
...
int minimax(int gameBoard[6][7], int depth, bool isMaximizing, int playerToken) {
    int winner = isWinner(playerToken);
    if (winner != FALSE) 
        return    ; //score of player token
    int score = -100000;
    for(int i = 0; i < 6; ++i) {
        for(int j = 0; j < 7; j++){
        if (gameBoard[i][j] == 0) {
            gameBoard[i][j] = playerToken;
            int thisScore = minimax(gameBoard, depth +1, FALSE, playerToken);
            gameBoard[i][j] = 0;
            if (thisScore > score) {
                score = thisScore;
            }
    ...
    for(int i = 0; i < 6; ++i) {
        ...
        if (gameBoard[i][j] == 0) {
            gameBoard[i][j] = playerToken;
            int thisScore = minimax(gameBoard, depth +1, TRUE, playerToken);
            gameBoard[i][j] = 0;
            if (thisScore > score) {
                score = thisScore;
            }
...
void botMove (int gameBoard[6][7], int token){
    ...
    for(int i = 0; i < 6; ++i) {
        for(int j = 0; j < 7; j++){
            if (gameBoard[i][j] == 0) {
            gameBoard[i][j] = token;
            int tempScore = minimax(gameBoard, 0, FALSE);
            gameBoard[i][j] = 0;   
            if (tempScore > score) {
                score = tempScore;
                bestmoveCol = i;
...
bool isValid(int gameBoard[6][7], int col){
