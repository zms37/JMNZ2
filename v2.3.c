    ...
    if (isMax)
    {
        ...
        for (int i = 1; i < 8; i++)
        {
            if (gameBoard[0][i] == 0)
            {
                ...
                gameBoard[j][i] = 1;
                best = max(best, miniMax(gameBoard, depth+1, !isMax, alpha, beta));
                gameBoard[j][i] = 0;
    ...
    else
    {
        ...
        for (int i = 1; i < 8; i++)
        {
            if (gameBoard[0][i] == 0)
            {
                ...
                gameBoard[j][i] = 2;
                best = min(best, miniMax(gameBoard, depth+1, !isMax, alpha, beta));
                gameBoard[j][i] = 0;
                beta = min(beta, best);
                if (beta <= alpha)
                {
...
// function isMax is true if current move is of maximizer, else false.
int isMax(int gameBoard[6][7])
{
    int count = 0;
    for (int i = 1; i < 8; i++)
...
// function to find the best move for the bot according to the choosen leve
int findBestMove(int gameBoard[6][7], int levelVal)
{
    int bestVal = -1000;
    int bestMove = -1;
    int alpha = -1000;
    ...
    if (levelVal == 1)
    {
        for (int i = 1; i < 8; i++)
        {
            if (gameBoard[0][i] == 0)
            {
                ...
                gameBoard[j][i] = 1;
                int moveVal = evaluate(gameBoard);
                gameBoard[j][i] = 0;
                if (moveVal > bestVal)
                {
                    bestMove = i;
    ...
    else if (levelVal == 2)
    {
        for (int i = 1; i < 8; i++)
        {
            if (gameBoard[0][i] == 0)
            {
                ...
                gameBoard[j][i] = 1;
                int moveVal = miniMax(gameBoard, 0, 0, alpha, beta);
                gameBoard[j][i] = 0;
                if (moveVal > bestVal)
                {
                    bestMove = i;
...
{
    int column = findBestMove(gameBoard, levelVal);
    int i = 0;
    while (gameBoard[i+1][column] == 0 && i < 5)
    {
        i++;
