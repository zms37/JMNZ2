...
int minimax(int gameBoard[6][7], int depth, bool isMaximizing, int playerToken) {
    ...
    return score;
        ...
        printf("minmax is thinking isMax2\n");
...
void botMove (int gameBoard[6][7], int token){
    ...
    int score = -1000000;
    for(int i = 0; i < 6; ++i) {
            printf("Bot is thinking for1...\n");

        for(int j = 0; j < 7; j++){
                printf("Bot is thinking for2...\n");

            if (gameBoard[i][j] == 0) {
                    printf("Bot is thinking if1...\n");

            gameBoard[i][j] = token;
            int tempScore = -minimax(gameBoard, 0, 0, token);
                printf("Bot is thinking minimax...\n");

            gameBoard[i][j] = 0;   
            if (tempScore > score) {
                    printf("Bot is thinkingif2...\n");
            ...
            printf("Bot is thinking out 2 ...\n");

    }
        printf("Bot is thinking out1...\n");
...
//algorithm
int evaluate(int gameBoard[6][7], int token){
    printf("evaluate is working...\n");
    int score = 0;
    //score center column
    int centerArray[7] = {3, 4, 5, 5, 4, 3};
    int centerCount = 0;
    for(int i = 0; i < 6; ++i) {
        printf("evaluate is working for1...\n");
        if (gameBoard[i][3] == token) {
            score += centerArray[i];
            centerCount++;
            printf("evaluate is working for if ...\n");
        }
        else if (gameBoard[i][3] == token) {
            score -= centerArray[i];
            centerCount++;
            printf("evaluate is working for else if ...\n");

        }
    }
    printf("evaluate is working out1...\n");
    //score horizontal
    for(int i = 0; i < 6; ++i) {
        printf("evaluate is working for 2...\n");

        int tokenCount = 0;
        for(int j = 0; j < 7; j++){
            printf("evaluate is working for 2.1...\n");
            if (gameBoard[i][j] == token) {
                tokenCount++;
                printf("evaluate is working for 2.1 if...\n");
            }
            else if (gameBoard[i][j] == token) {
                tokenCount--;
                printf("evaluate is working for 2.1 else if...\n");
            }
        }
        score += tokenCount;
    }
    printf("evaluate is working out2...\n");
    //score vertical
    for(int i = 0; i < 7; ++i) {
        printf("evaluate is working for 3...\n");
        int tokenCount = 0;
        for(int j = 0; j < 6; j++){
            printf("evaluate is working for 3.1...\n");
            if (gameBoard[j][i] == token) {
                tokenCount++;
                printf("evaluate is working for 3.1 if...\n");
            }
            else if (gameBoard[j][i] == token) {
                tokenCount--;
                printf("evaluate is working for 3.1 else if...\n");
            }
        }
        score += tokenCount;
    }
    printf("evaluate is working out3...\n");
    //score positive sloped diagonal
    for(int i = 0; i < 3; ++i) {
        printf("evaluate is working for 4...\n");
        for(int j = 0; j < 4; j++){
            printf("evaluate is working for 4.1...\n");
            int tokenCount = 0;
            for(int k = 0; k < 4; k++){
                printf("evaluate is working for 4.1.1...\n");
                if (gameBoard[i+k][j+k] == token) {
                    tokenCount++;
                    printf("evaluate is working for 4.1.1 if...\n");
                }
                else if (gameBoard[i+k][j+k] == token) {
                    tokenCount--;
                    printf("evaluate is working for 4.1.1 else if...\n");
                }
            }
            score += tokenCount;
        }
    }
    printf("evaluate is working out4...\n");
    //score negative sloped diagonal
    for(int i = 0; i < 3; ++i) {
        printf("evaluate is working for 5...\n");
        for(int j = 0; j < 4; j++){
            printf("evaluate is working for 5.1...\n");
            int tokenCount = 0;
            for(int k = 0; k < 4; k++){
                printf("evaluate is working for 5.1.1...\n");
                if (gameBoard[i+k][j-k] == token) {
                    tokenCount++;
                    printf("evaluate is working for 5.1.1 if...\n");
                }
                else if (gameBoard[i+k][j-k] == token) {
                    tokenCount--;
                    printf("evaluate is working for 5.1.1 else if...\n");
                }
            }
            score += tokenCount;
        }
    }
    printf("evaluate is working out5...\n");
    return score;
}
